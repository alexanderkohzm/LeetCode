...
function canFinishAdjacencyList(
  numCourses: number,
  prerequisites: number[][]
): boolean {
  // we should first create a map of all the courses and their prerequisites.
  // this will be an adjacency list

  const adjacencyList = {} as { [key: number]: Array<number> };

  // populate the adjacency list
  for (const courses of prerequisites) {
    const [course, prereq] = courses;
    if (adjacencyList[course]) {
      // then we should just push in the pre-req into that course
  ...
  // now we need to create the depthFirstSearch function
  // this depthFirstSearch will loop through all the courses in the adjacencyList

  // it will first add the current course we're visiting to the "visitedSet".
  // This set keeps track to see if we're in a loop - where a course's prerequiresite has a prerequisite that is itself

  // we will then call DFS through all the prerequisites
  // if the prerequisites are empty or non-existent, then we know this pathway is possible
  // if not, we return false

  // once we have finished the DFS, we remove the visited set because we know that there's no loop
  // AND we can set that course pre-requisite to [] because that course has a valid path
  const visitedSet = new Set<number>();
  function depthFirstSearch(course: number) {
    if (visitedSet.has(course)) return false;

    if (
      (adjacencyList[course] && adjacencyList[course].length === 0) ||
      !adjacencyList[course]
    )
      return true;

    const prerequisitesForThisCourse = adjacencyList[course];

    // add it to the set
    visitedSet.add(course);
    for (const prereqs of prerequisitesForThisCourse) {
      if (!depthFirstSearch(prereqs)) return false;
    }
